...
from proyecto.forms import ContactoForm
...
def buscar(request):
	if "busca" in request.GET and request.GET["busca"]:
		...
		variedades = Variedad.objects.filter(estilo__icontains=consulta)
		return render(request, 'resultados.html', {'res': variedades})
	else:
...
def contacto(request):
	if request.method == 'POST':
		...
		if formulario.is_valid():
			...
			correo = EmailMessage(titulo, contenido, to=['alanpizzo15@gmail.com'])
			try:
				correo.send()
